...
$(document).ready(function () {
    ...
    bindCustomLengthSelector(storeTable);
    ...
    $('#addStoreBtn').click(function() {
        resetForm();
        $('#storeModalLabel').text('Add Store Ledger');
...
// ==========================
function saveStore() {
    if (!$('#storeForm')[0].checkValidity()) {
        $('#storeForm')[0].reportValidity();
        return;
    }
    ...
    $.ajax({
        ...
        error: function(err) {
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Failed to create store: ' + (err.responseJSON?.error || err.statusText)
            });
        }
    });
}
...
function editStore(id) {
    $.ajax({
        ...
        success: function(data) {
            resetForm();
            $('#storeModalLabel').text('Edit Store Ledger');
            $('#storeNameInput').val(data.store_name);
    ...
    $.ajax({
        ...
        error: function(err) {
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Failed to update store: ' + (err.responseJSON?.error || err.statusText)
            });
        }
    });
}
            ...
            $.ajax({
                ...
                success: function() {
                    Swal.fire({
                        icon: 'success',
                        title: 'Deleted!',
                        text: 'Store record has been deleted.',
                        timer: 1500,
